...
//  }
 ...
 //PVector normalPoint = getNormalPoint(predictLoc,path.start,path.end);
 ...
 //   strokeWeight(4);
 //   point(predict.x,predict.y);
 //   popMatrix();
    
 //   strokeWeight(1);
 //   line(location.x+predict.x,location.y+predict.y,normalPoint.x,normalPoint.y);
 //   strokeWeight(4);
...
PVector cohesion(ArrayList<Vehicle> v){
  ...
  void applyBehaviors(ArrayList<Vehicle> v){
    ...
    PVector seek = seek(mouse_target);
