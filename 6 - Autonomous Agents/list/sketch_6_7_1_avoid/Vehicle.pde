...
PVector steer(PVector location_,PVector target_, PVector velocity_){
  ...
  void seekAndAvoid(PVector target){
    PVector force = steer(location, target, velocity);
    PVector ahead = location.copy();
    ahead.add(velocity);
    ahead.add(force);
    ...
    PVector distance = PVector.sub(mouse_target,ahead);
  ...
  // line(location.x,location.y,ahead.x, ahead.y);
    ...
    if (diff > 0){
     ...
     //force = steer(ahead, mouse_target, velocity);
     //force.mult(-1);
     //applyForce(force);
     
     PVector desired = PVector.sub(ahead,mouse_target);
    desired.setMag(max_speed);
    
    float d = desired.mag();
    
    force = PVector.sub(desired,velocity);
    float f = map(d,0,threshold,max_force,0);
    force.setMag(f);
